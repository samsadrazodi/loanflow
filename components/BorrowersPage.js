...
export default function BorrowersPage({ onToast }) {
  ...
  if (selected) return <BorrowerProfile borrower={selected} onBack={() => setSelected(null)} />;

  return (<>
    <section className="stats-row" style={{gridTemplateColumns:'repeat(3,1fr)'}}>
      {[{ label:'Total Borrowers',value:'962',change:'+24 this month',icon:Icons.users,color:'blue' },{ label:'Active Loans',value:'1,841',change:'+5.6% from last month',icon:Icons.activity,color:'green' },{ label:'Avg. Credit Score',value:'724',change:'+3 pts from last quarter',icon:Icons.barChart,color:'purple' }].map((s,i)=>(
    ...
    <section className="toolbar">
      <div className="toolbar-left"><div className="search-input">{Icons.search}<input placeholder="Search borrowers by name or email..." value={search} onChange={e=>setSearch(e.target.value)} /></div></div>
    ...
    <section className="table-container">
      ...
      <div style={{overflowX:'auto'}}><table><thead><tr><th>Borrower</th><th>Email</th><th>Active Loans</th><th>Total Borrowed</th><th>Credit Score</th><th>Since</th><th>Risk</th></tr></thead>
      <tbody>{filtered.map(b=>(
        <tr key={b.email} onClick={()=>setSelected(b)}>
